...
from modbus_config import FUNCTIONS, REGISTERS
...
class ModbusClient:
    ...
    def write_register(self, address: int, value: int, unit_id: int = 1) -> bool:
        ...
        try:
            ...
            if result.isError():
                logger.error(f"Error writing value {value} to register {address}")
                return False
    ...
    def read_all_prices(self) -> Dict:
        ...
        for i in range(1, 6):
            ...
            if price_data:
                ...
                logger.info(
                    f"Program {i}: regular={price_data['regular_price']}, loyalty={price_data['loyalty_price']}")
            else:
