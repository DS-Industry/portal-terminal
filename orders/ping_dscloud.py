...
def dscloud_job():
    ...
    try:
        ...
        # Всегда получаем ссылку на активный заказ, если он есть
        processing_order = WashOrder.objects.filter(status=WashOrder.Status.PROCESSING).first()
        any_processing_order_exists = processing_order is not None
        current_processing_order_id = str(processing_order.transaction_id) if processing_order else None
        
        if any_processing_order_exists:
            print(f"[DS] Мойка занята (заказ {current_processing_order_id}). Ограниченный режим работы.")
        else:
            print("[DS] Мойка свободна. Выполняем полный пинг и проверку оплаты.")

        response_data = send_data_to_dscloud()
        
        if not any_processing_order_exists and response_data:
        ...
        if not any_processing_order_exists:
            ...
            if processing_order and current_processing_order_id != _last_processing_order_id:
                ...
                # 3) Заказ завершён, нужно обнулить gvl_sum
                print(f"[DS-CLEANUP] Заказ {_gvl_sent_for} завершён. Обнуляем GVL_SUM в БД.")
            ...
            if processing_order and current_processing_order_id != _last_processing_order_id:
                print(f"[DS-PROCESSING-LIMITED] Обнаружен активный заказ в статусе PROCESSING: {current_processing_order_id}")
                expected_sum = int(processing_order.program_price)
                ...
                if ts:
                    ...
                    print(f"[DS-PROCESSING-LIMITED] GVL_SUM обновлён в БД на сумму заказа: {expected_sum}")
                    ...
                    while not confirmed and retries < max_retries:
                        response_data = send_data_to_dscloud()
                        if response_data and response_data.get('GVLSum') == str(expected_sum):
                            print(f"[DS-PROCESSING-LIMITED] Подтверждение получения суммы {expected_sum} от DScloud получено.")
                            confirmed = True
                            _gvl_sent_for = current_processing_order_id
                            _last_processing_order_id = current_processing_order_id
                        else:
                            print(f"[DS-PROCESSING-LIMITED] Ожидание подтверждения суммы {expected_sum} от DScloud... (Попытка {retries + 1}/{max_retries})")
                            if response_data:
                                print(f"[DS-PROCESSING-LIMITED] Получен ответ: {response_data}")
                            retries += 1
                            if retries == max_retries:
                                print(f"[DS-PROCESSING-LIMITED] ОШИБКА: Не удалось получить подтверждение суммы {expected_sum} от DScloud после {max_retries} попыток.")

            elif not processing_order and _gvl_sent_for:
                # 3) Заказ завершён, нужно обнулить gvl_sum
                print(f"[DS-CLEANUP-LIMITED] Заказ {_gvl_sent_for} завершён. Обнуляем GVL_SUM в БД.")
                ...
                if ts:
                    ...
                    print(f"[DS-CLEANUP-LIMITED] GVL_SUM обнулён в БД.")
                    ...
                    while not confirmed_zero and retries < max_retries:
                        response_data = send_data_to_dscloud()
                        if response_data and response_data.get('GVLSum') == '0':
                            print(f"[DS-CLEANUP-LIMITED] Подтверждение обнуления суммы от DScloud получено.")
                            confirmed_zero = True
                            _gvl_sent_for = None
                            _last_processing_order_id = None
                            # Сбрасываем флаг мобильной оплаты при завершении любого заказа
                            _mobile_order_confirmed = False 
                        else:
                            print(f"[DS-CLEANUP-LIMITED] Ожидание подтверждения обнуления суммы от DScloud... (Попытка {retries + 1}/{max_retries})")
                            if response_data:
                                print(f"[DS-CLEANUP-LIMITED] Получен ответ: {response_data}")
                            retries += 1
                            if retries == max_retries:
                                 print(f"[DS-CLEANUP-LIMITED] ОШИБКА: Не удалось получить подтверждение обнуления суммы от DScloud после {max_retries} попыток.")
